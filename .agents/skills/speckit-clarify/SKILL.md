...
1. Run the prerequisite script from repo root **once** in combined JSON and paths-only mode, then parse the minimal JSON payload fields:
   - Bash: `bash .specify/scripts/bash/check-prerequisites.sh --json --paths-only`
   - PowerShell: `powershell -ExecutionPolicy Bypass -File .specify/scripts/powershell/check-prerequisites.ps1 -Json -PathsOnly`
   - `FEATURE_DIR`
   - `FEATURE_SPEC`
